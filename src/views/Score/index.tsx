...
export default function Score() {
  ...
  const completionPercent = Math.max(0, Math.min(100, snapshot.completion * 100));
    ...
    <>
      <div className="score-overlay">
        ...
        <div className="score-overlay__meta">
          ...
          <span>
            Progreso: {completionPercent.toFixed(0)}%
          </span>
          <span>
            Actual: {snapshot.currentNodeId ?? "-"}
          </span>
        </div>
      </div>
      ...
      <div className="score-overlay score-overlay--right">
        ...
        <div className="score-overlay__value">{completionPercent.toFixed(0)}%</div>
        <div className="score-overlay__meta">
          <span>Repetición objetivo: 1</span>
          <span>Estado: {snapshot.completed ? "Completada" : "En progreso"}</span>
          {snapshot.gridTotalKeypoints > 0 && (
            <>
              <span>Grid: {snapshot.gridScore}%</span>
              <span>
                Grid KP: {snapshot.gridMatchedKeypoints}/{snapshot.gridTotalKeypoints}
              </span>
