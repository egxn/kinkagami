...
function Canvas() {
  const { videoRef, stream, detector, modelLoading, streamReady } = usePoseContext();
  const canvasRef = useRef<HTMLCanvasElement>(null);

  // Handle poses detected
  const handlePosesDetected = useCallback((poses: poseDetection.Pose[]) => {
    ...
    const playVideo = async () => {
      try {
        ...
        if (error instanceof DOMException && error.name === 'AbortError') {
          logger.log('Canvas', 'Video playback aborted - stream may not be ready yet');
        } else {
  ...
  useEffect(() => {
    ...
    const updateCanvasDimensions = () => {
      ...
      logger.log('Canvas', `Canvas dimensions: ${canvas.width}x${canvas.height}`);
    };
