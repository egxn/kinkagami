...
import type { HandEstimator } from "../../types/inference";

export type HandPoseDetector = HandEstimator;

interface UseLocalHandPoseReturn {
...
const initializeSharedDetector = async (
  ...
  sharedInitPromise = (async () => {
    try {
      ...
      logger.log("useLocalHandPose", sharedStatus);
      ...
      if (!currentBackend || currentBackend === "cpu" || currentBackend !== preferredBackend) {
        try {
          ...
          await tf.ready();
          logger.log("useLocalHandPose", `${tf.getBackend()} backend is ready`);
        } catch {
          const fallback = preferredBackend === "webgl" ? "wasm" : "webgl";
          logger.log("useLocalHandPose", `${preferredBackend} failed, falling back to ${fallback}`);
      ...
      logger.error("useLocalHandPose", "Error:", message);
...
export const useLocalHandPose = (enabled = true): UseLocalHandPoseReturn => {
